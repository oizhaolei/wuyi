...
function ome_create_reimburse(){
    ...
    if ($data['cur_money'] >= $order['payed_all']) {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_RETURNED, SS_UNSHIPPED, PS_UNPAYED, PS_UNRETURNED, $action_note);
        ...
        update_order($order['order_id'], $arr);
        /* 记录log */
        order_action($order['order_sn'], OS_RETURNED, SS_UNSHIPPED, PS_UNPAYED, RS_UNRETURNED, $action_note,'system');
    }
...
function send_refund_to_matrix($data){
    ...
    include_once(ROOT_PATH . 'includes/cls_matrix.php');
    $matrix = new matrix;
    $matrix->send_refund_to_matrix($msg);
    $matrix->createOrder($data['order_id']);
}
