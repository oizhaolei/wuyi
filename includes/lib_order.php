 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $days= 1, $spec = array(), $parent = 0, $rec_type = CART_GENERAL_GOODS)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_number = '$num' , goods_days = '$days'" .
...
function merge_order($from_order_sn, $to_order_sn)
{
    ...
    elseif ($to_order['return_status'] != RS_RETURNED)
    {
        return sprintf($GLOBALS['_LANG']['rs_not_unreturned'], $to_order_sn);
    }
