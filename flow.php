...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    {
        flow_update_cart($_POST['goods_number'], $_POST['goods_days']);
    }
...
elseif ($_REQUEST['step'] == 'add_package_to_cart')
{
    ...
    if (!is_numeric($package->days) || intval($package->days) <= 0)
    {
        $result['error']   = 1;
        $result['message'] = $_LANG['invalid_days'];
    }
...
elseif ($_REQUEST['step'] == 'repurchase') {
    ...
    foreach ($order_goods as $goods) {
        ...
        if (!addto_cart($goods['goods_id'], $goods['goods_number'], $goods['goods_days'], $spec, $goods['parent_id'], CART_REPURCHASE)) {
            $result = false;
 ...
 * @return  void
 */
function flow_update_cart($arr_num, $arr_days)
{
    /* 处理 */
    ...
    foreach ($arr_days AS $key => $val)
    {
        ...
        $sql = "UPDATE " .$GLOBALS['ecs']->table('cart').
                " SET goods_days = '$val' WHERE rec_id='$key' AND session_id='" . SESS_ID . "'";
        $GLOBALS['db']->query($sql);
    }
