...
if ($_REQUEST['act'] == 'list')
{
    ...
    $allow_set_points = $_CFG['integrate_code'] == 'wuyi' ? 0 : 1;
...
if ($_REQUEST['act'] == 'install')
{
    ...
    if ($_GET['code'] == 'wuyi')
    {
        $sql = "UPDATE " .$ecs->table('shop_config'). " SET value = 'wuyi' WHERE code = 'integrate_code'";
        $db->query($sql);
...
if ($_REQUEST['act'] == 'check_user')
{
    ...
    $domain = empty($_GET['domain']) ? '@wuyi' : trim($_GET['domain']);
    if ($size <2)
    {
        $size = 2;
    }
