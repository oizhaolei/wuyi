...
if ($_REQUEST['act'] == 'order_query')
{
    ...
    $smarty->assign('rs_list', get_status_list('return'));
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $order['status']        = $_LANG['os'][$order['order_status']] . ',' . $_LANG['ps'][$order['pay_status']] . ',' . $_LANG['ss'][$order['shipping_status']] . ',' . $_LANG['rs'][$order['return_status']];
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $row['return_status'] = $_LANG['rs'][$row['return_status']];
    ...
    if ('confirm' == $operation)
    {
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                /* 记录log */
                order_action($order['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, PS_UNPAYED, RS_UNRETURNED, $action_note);
    ...
    elseif ('invalid' == $operation)
    {
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                /* 记录log */
                order_action($order['order_sn'], OS_INVALID, SS_UNSHIPPED, PS_UNPAYED, RS_UNRETURNED, $action_note);
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('unship' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], $order['order_status'], SS_UNSHIPPED, $order['pay_status'], $order['return_status'], $action_note);
    ...
    elseif ('restore' == $operation)
    {
        ...
        $arr['restore_time']       = time(); // 归还时间
        update_order($order_id, $arr);

        /* 记录log */
        order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], RS_RETURNED, $action_note);
    }
    /* 取消 */
    elseif ('cancel' == $operation)
    {
    ...
    elseif ('return' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], OS_RETURNED, SS_UNSHIPPED, PS_UNPAYED, RS_UNRETURNED, $action_note);
 ...
 * @return  array   可执行的操作  confirm, pay, unpay, prepare, ship, unship, receive, cancel, invalid, return, drop, restore
        ...
        else
        {
            ...
            else
            {
                ...
                if ($priv_list['ss'])
                {
                    ...
                    elseif (SS_RECEIVED == $ss && RS_UNRETURNED == $rs)
                    {
                        $list['restore'] = true; // 归还
                    }
                    if (!$is_cod)
                    {
                        $list['unship'] = true; // 设为未发货
